let vm = new Vue({
    ...
    methods: {
        ...
        save_address(){
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id ) {
                ...
                if (this.editing_address_index === '') {
                    ...
                    axios.post(url, this.form_address, {
                        ...
                        .then(response => {
                            ...
                            if (response.data.code == '0') {
                                ...
                                location.href = '/login/?next=/user/addresses/';
                            } else {
                    ...
                    // 修改地址
                    let url = '/user/addresses/' + this.addresses[this.editing_address_index].id + '/';
        ...
        delete_address(index){
            let url = '/user/addresses/' + this.addresses[index].id + '/';
            axios.delete(url, {
                headers: {
                    'X-CSRFToken':getCookie('csrftoken')
                },
        ...
        // 设置默认地址
        set_default(index){
            let url = '/user/addresses/' + this.addresses[index].id + '/default/';
            axios.put(url, {}, {
                headers: {
                    'X-CSRFToken':getCookie('csrftoken')
                },
        ...
        save_title(index){
            if (!this.new_title) {
                ...
                let url = '/user/addresses/' + this.addresses[index].id + '/title/';
                axios.put(url, {
